...
class LDA_model:
    ...
    def result_plot(self,num_topics_list):
        ...
        # 在第一个小区域中绘制第一条曲线
        axes[0].plot(num_topics_list, self.pp_word_list, label='word')
        axes[0].plot(num_topics_list, self.pp_char_list, label='char')
        axes[0].set_title('perplexity')
        axes[0].legend()

        # 在第二个小区域中绘制第二条曲线
        axes[1].plot(num_topics_list, self.cv_word_list, label='word')
        axes[1].plot(num_topics_list, self.cv_char_list, label='char')
        axes[1].set_title('coherence')
        axes[1].legend()
        plt.savefig('word_char2.jpg')  #保存成jpg格式
        plt.show()
